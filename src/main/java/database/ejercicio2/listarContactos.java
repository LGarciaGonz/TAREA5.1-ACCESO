...
public class listarContactos {
    public static void listadoContactos() {
        ...
        try (Connection miCon = database.ConectionsPool.conectar()) {
            ...
            // Preparar la consulta para listar los nombres.
            String listCont = "SELECT A.NOMBRE, A.TELEF FROM AGENDA A";
            Statement stmt = miCon.createStatement();
            ResultSet rs = stmt.executeQuery(listCont);

            // Mostrar los nombres de los contactos obtenidos.
            while (rs.next()){
                String contRs = rs.getString("A.NOMBRE");
                int telefonos = rs.getInt("A.TELEF");
                System.out.println("\n- " + contRs + ": " + telefonos);
            }

        } catch (SQLException e) {
            // Manejar errores de SQL
            System.err.println("\n>>> No se han podido listar los contactos. ERROR: " + e.getMessage());
        }
